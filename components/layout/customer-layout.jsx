...
import { FileText, Home, LogOut, Menu, PlusCircle, User } from "lucide-react"
...
export default function CustomerLayout({ children }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-gray-900 transition-colors duration-200">
      <header className="bg-gradient-to-r from-green-600 via-blue-500 to-yellow-500 text-white shadow-md">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="hidden md:inline-block text-sm">
              {t("welcome")}, {user?.name || t("customer")}
            </span>
...
function DesktopNavItems({ t }) {
  return (
    <>
      ...
      <Link href="/cliente/garantias">
        <div className="flex items-center px-4 py-3 text-gray-300 hover:bg-gray-800 hover:text-white rounded-md transition-colors">
          <FileText className="h-5 w-5 mr-3" />
          <span>{t("myWarranties")}</span>
        </div>
      </Link>

      <Link href="/cliente/garantias/nueva">
        <div className="flex items-center px-4 py-3 text-gray-300 hover:bg-gray-800 hover:text-white rounded-md transition-colors">
          <PlusCircle className="h-5 w-5 mr-3" />
          <span>{t("newWarranty")}</span>
        </div>
      </Link>
